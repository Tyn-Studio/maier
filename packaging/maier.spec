...
on tag pushes, via `uv run pyinstaller packaging/maier.spec`
...
MAIER_PKG = SRC_ROOT / "maier"

block_cipher = None

# --- Data files -------------------------------------------------------
# (source, destination-relative-to-bundle) pairs. Destinations mirror the
# package layout so `maier.settings` (APP_DIRS template loading,
# staticfiles finders, migration discovery) finds them exactly where it
# would in a normal `pip install`.
from PyInstaller.utils.hooks import collect_data_files

datas = [
    (str(MAIER_PKG / "core" / "templates"), "maier/core/templates"),
    (str(MAIER_PKG / "core" / "static"), "maier/core/static"),
    (str(MAIER_PKG / "core" / "migrations"), "maier/core/migrations"),
]
# pyicloud's dependency chain ships package data PyInstaller doesn't collect
# by default -- fido2's public_suffix_list.dat crashed the first Maier-named
# bundle at import time (2026-08-24). Collect data for both defensively.
...
# with `No module named 'maier.settings'`. Collect the whole maier package
...
if sys.platform == "darwin":
    app = BUNDLE(
        ...
        name=f"{APP_NAME}.app",
        icon=None,
        bundle_identifier=None,  # TODO(bundle-id): reverse-DNS id once there's a real domain
    )
