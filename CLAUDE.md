...
9. Remote (iCloud) photos have no local file until selected: their durable state lives in `{folder}/icloud-state/{account}.json` (NOT in `.maier/` — it must survive cache deletion and travel with the folder). Originals download only into `selected/{account}/…`; once downloaded they are ordinary local files.
