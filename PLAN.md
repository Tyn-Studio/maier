...
- [x] **v0.1.5** (2026-08-25) — On-demand thumbs actually work + flicker fixed. Live findings: (1) v0.1.3's socket.setdefaulttimeout NEVER applied to pyicloud (requests passes explicit timeout=None, overriding it) — real fix wraps session.send forcing 30s; (2) single-asset recordName lookups tarpit INDEFINITELY (150s+ probe) — unusable; replaced with `scan_window(offset, limit)`: DB mirror computes a date's rank (photos-newer count), jumps the positional enumeration straight there — probed: landed exactly on target 8,096 deep, 60 assets/5.2s, thumb 1.1s; preview_upgrade warms per-account windows for pending thumb batches, then downloads via cached CDN URLs. (3) Flicker: poller wrapped the no-store placeholder img, refetching it every 2s swap — now an invisible span polls and the img is OOB-swapped exactly once when ready.
- [x] **T34 / v0.1.4** (2026-08-25) — On-demand grid thumbnails: rendering a grid page enqueues immediate fetches (own client, 3 workers, tier-keyed single-flight in preview_upgrade) for exactly the visible remote photos missing thumbs; cells carry a self-healing poller (~40s cap) that swaps the real thumb in as it lands. Profiling first (real 41k DB): grid 35ms, cached preview serve 0.5ms — serving was never the problem; cold ranges just had no priority path. Known gap: set-status cell re-render doesn't set thumb_pending (degrades to plain img until next page load). Follow-up queued: local-video poster thumbnails.
- [x] **v0.1.3** (2026-08-24) — THE preview fix, live-diagnosed: (1) Apple TARPITS throttled album-lookup queries and pyicloud has no timeouts → workers hung in ssl.read forever (stack-dumped); process-wide socket timeout added. (2) pyicloud's direction naming is INVERTED — ASCENDING = newest-first (probed: 2026-08-23 in 3s); enumeration now newest-first so recent working ranges cache in the first minute. (3) Previews fetch FROM the enumeration stream (cached asset → direct CDN, the only never-failed path); album lookups only for post-enum leftovers, timeout-bounded. Settings comment leak fixed (3rd `{# #}` bite) + template hygiene guard test.
- [x] **v0.1.2** (2026-08-24) — Preview workers get their OWN iCloud session (independent album pagination): the v0.1.1 deferral avoided contention but made first-run in-range previews wait behind the whole enumeration — same gray grid. Now previews flow from the first seconds, range-priority, ~2s/item uncontended. Priority tests fixed to assert dispatch, not parallel-completion order.
- [x] **T32/T33 v0.1.1** (Luis, 2026-08-24) — Preview workers no longer starve behind enumeration (cache-miss backlog deferred until enumerated; root cause of all-day "gray grid": `photos.all.get()` contends with active enumeration). Boot auto-resumes pulls for live sessions. **Working range = priority, not fence**: in-range previews first (grid order), then whole-library backfill (newest-first, post-enumeration). Filter bar aligned (.filter-field units, baseline-aligned buttons); working-range chip removed — grid From/To inputs ARE the persistent range (debounced auto-save). Integration wait budget 5→15s (suite-load flake).
- [x] **T31 Settings UX fixes + v0.1.0 SHIPPED** (Luis, 2026-08-24) — auto-save settings (no button, partial updates, "Saved ✓"), single Apple-style add-account flow, FileDialog.FOLDER shim. **v0.1.0 published**: github.com/Tyn-Studio/maier/releases/v0.1.0 (Maier-macos.zip, CI-built + smoke-tested; update feed live). Release took 4 tags: flaky preview test → token permissions 403 → format check → green. Two more flaky tests hardened (async-enqueue assertion window, worker-reset leak).
